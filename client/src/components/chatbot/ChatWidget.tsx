...
const QUICK_ACTIONS = [
  { label: 'Check Investments', icon: TrendingUp },
  { label: 'Portfolio Performance', icon: PieChart },
  { label: 'Loan Eligibility', icon: Landmark },
  { label: 'Insurance Plans', icon: ShieldCheck },
  { label: 'Savings Advice', icon: PiggyBank },
...
export default function ChatWidget({ onClose }: ChatWidgetProps) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="bg-green-950 text-white p-5 pb-6 flex items-start justify-between relative shrink-0 rounded-t-[1.3rem]">
        {/* Background Graphic */}
        <div className="absolute right-0 bottom-0 opacity-40 pointer-events-none">
          <svg width="180" height="60" viewBox="0 0 180 60" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M10 50 L40 40 L70 45 L100 20 L130 35 L160 5 L175 0" stroke="#CCAFA1" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
            <path d="M165 5 L175 0 L175 10" stroke="#CCAFA1" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
            <rect x="35" y="45" width="10" height="20" fill="#CCAFA1" />
            <rect x="65" y="48" width="10" height="17" fill="#CCAFA1" />
            <rect x="95" y="25" width="10" height="40" fill="#CCAFA1" />
            <rect x="125" y="40" width="10" height="25" fill="#CCAFA1" />
            <rect x="155" y="10" width="10" height="55" fill="#CCAFA1" />
          </svg>
        </div>

        <div className="flex gap-4 items-center z-10">
          {/* Avatar */}
          <div className="w-12 h-12 bg-gradient-to-br from-gold-400 to-green-700 rounded-full flex items-center justify-center border-2 border-slate-700 shadow-inner">
             <Bot size={24} className="text-white" />
          </div>
          <div>
            <h2 className="font-bold text-xl flex items-center gap-1.5 tracking-tight">
              FinAI <span className="text-gold-400">✨</span>
            </h2>
            <div className="text-[11px] !text-white mt-0.5 font-medium tracking-wide">
              Powered by MK Investors
            </div>
      ...
      {/* Message Area */}
      <div className="flex-1 overflow-y-auto p-5 bg-[#f8fafc] chat-scroll flex flex-col relative">
              ...
              <div className="grid grid-cols-2 gap-2 mt-1 mb-6 ml-[3.25rem] pr-2">
                {QUICK_ACTIONS.map((action, i) => (
                  <button
                    ...
                    disabled={isLoading}
                    className="flex items-center gap-2.5 bg-white border border-slate-200 hover:border-gold-400 hover:bg-green-100/30 hover:shadow-sm transition-all rounded-xl p-2.5 text-left group disabled:opacity-50"
                  >
                    <div className="w-7 h-7 rounded-lg bg-slate-50 text-green-700 flex items-center justify-center shrink-0 group-hover:bg-green-100 group-hover:text-green-700 transition-colors">
        ...
        {isLoading && (
          <div className="flex justify-start mb-6">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-gold-400 to-green-700 flex items-center justify-center shrink-0 mr-3 shadow-sm border border-slate-200">
